...
int obtener_fd_memoria(instr_t *i){
	...
	case EC:
		...
		if(list_size(criterio_eventual_consistency->lista_memorias)>0){
...
//	fd_out_inicial = 0; ????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
	/*
	 * SOY UN FORROOOOOO MIRA LO Q PUSE LPMMMMM ALTO BOBO CCON RAZON NO ANDABA
	 */
	while(1)
	{
