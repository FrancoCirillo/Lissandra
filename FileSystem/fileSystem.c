...
//		loggear_error(string_from_format("-------------------Entre a dump-------------------"));
		...
		loggear_debug(string_from_format("Al temporal Dump%d.tmp se le asigno el bloque %d\n", nro_dump, nro_bloque));
...
//			printf("Ruta Bloque: %s\nRuta TMP: %s\n", ruta_bloque, ruta_tmp);
	...
	registro_t* registro = crearRegistro(432423412, 3, "ChauTmbVoyEnT2");
...
void pruebaGeneral() {
	...
	loggear_info(string_from_format("Ya termino el dumpeo"));
...
void imprimir_donde_corresponda(cod_op codigoOperacion, instr_t* instruccion, t_list* listaParam, char* remitente) {
	...
	case CONSOLA_KERNEL:
//		puts("La instruccion la pidio la consola del Kernel");
...
//		puts("La instruccion la pidio la consola de Memoria");
...
//		puts("La instruccion la pidio la consola del FS");
		...
		//free(miInstruccion); GENERAN INVALID READ
